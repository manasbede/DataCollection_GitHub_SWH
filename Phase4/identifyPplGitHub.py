...
fileLocation=rf"C:\Users\mbedek\Documents\GitHub\DataCollection_GitHub_SWH\Dataset\GitHub"
cwd=os.getcwd()
...
def main():
    ...
    for fileToCheck in fileData:
        ...
        #time.sleep(10)
        try:
            open(f"{cwd}\\Phase4\\GitHubKeywords.txt","a").write(f"{fileToCheck.split('.')[0]}:{keywords}\n")
        except:
            open(f"{cwd}\\Phase4\\issueGitHub.txt","a").write(f"{fileToCheck.split('.')[0]}\n")
